...
public class RegeneratorManager {
    ...
    private void regenerateAsync(RegeneratorBlock rb) {
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                // ── Paso 1: escaneo de solo lectura ──────────────────
                // Recolectamos las posiciones con cultivos inmaduros antes
                // de abrir un EditSession de escritura. Si no hay ninguno,
                // salimos sin crear ninguna sesión de escritura.
                List<BlockVector3> toGrow = new java.util.ArrayList<>();

                try (EditSession readSession = WorldEdit.getInstance()
                    ...
                    for (BlockVector3 pos : region) {
                        BlockState state = readSession.getBlock(pos);
                ...
                // Sin cultivos inmaduros — no abrir EditSession de escritura
                if (toGrow.isEmpty()) return;

                // ── Paso 2: escritura solo de las posiciones necesarias ──
                try (EditSession writeSession = WorldEdit.getInstance()
                        .newEditSessionBuilder()
                        .world(weWorld)
                        .fastMode(true)
                        .limitUnlimited()
                    ...
                    for (BlockVector3 pos : toGrow) {
                        BlockState state = writeSession.getBlock(pos);
                        BlockType type   = state.getBlockType();
                        if (type == null) continue;
                        ...
                        writeSession.setBlock(pos, mature);
                        grown++;
                    }
